...
class BaseConfig:
    ...
    @staticmethod
    def getVariable(name: str, defaultValue: Any = "", deserializeJson: bool = False) -> Any:
        """
        Retrieve the value of an environment variable from local env only.

        Args:
            name: The name of the variable.
            defaultValue: Default value if not found. Defaults to an empty string ("").
        ...
        if value is None:
            logger.warning("Environment variable '%s' is not found in the environment. Using default value: '%s'", name, defaultValue)
            return defaultValue
